...
@pytest.mark.parametrize(
    "frame_rate", (22050, 41100), ids=("frame_rate=22050", "frame_rate=41100")
)
@pytest.mark.parametrize(
    "max_duration",
    (1, 1.5),
...
def test_wavelint(
    ...
    expected_messages = [
        (
            "Found lower duration (0.5) than allowed (1.2)"
            " at file /tmp/wavelint__test_0.wav"
        ),
    ]
    for i in range(5):
        ...
        time_to_frame = (
            ...
            if nchannels == 1
            else stereo_ttf_gen(frequencies=(220, 440), **kwargs)
        )

        Sound.from_datatimes(time_to_frame, fps=frame_rate,).with_duration(
            duration
        ).save(filename)
